<?php get_header(); ?>
    ...
    <?php
    $card_class = is_search() ? 'c-card c-card--search' : 'c-card';
    $s = get_search_query();
    ...
    if ($s) {
        $tag = get_term_by('name', $s, 'post_tag');
        if (!$tag) {
            $tag = get_term_by('slug', $s, 'post_tag');
        }
        if ($tag) {
         ...
         <div id="post-<?php the_ID(); ?>" <?php post_class('c-card__item'); ?>>
            <figure class="c-card__image">
                <?php if (has_post_thumbnail()) :?>
                    <?php the_post_thumbnail('medium', ['alt' => is_search() ? '検索結果サムネイル' :  esc_attr(get_the_title()),
                  ]) ; ?>
                <?php else : ?>
